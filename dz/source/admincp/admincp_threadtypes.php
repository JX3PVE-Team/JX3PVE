...
EOT;
	...
	if(!submitcheck('editsubmit')) {
		...
		foreach(array('number', 'text', 'radio', 'checkbox', 'textarea', 'select', 'calendar', 'email', 'url', 'image', 'range','face') as $type) {
		...
		if($_GET['sortid']) {
			...
			if(is_array($sortoptionarray)) foreach($sortoptionarray as $optionid => $option) {
				...
				if($option['search']) {
					...
					} elseif(in_array($option['type'], array('number', 'text', 'email', 'calendar', 'image', 'url', 'textarea', 'upload', 'range','face'))) {
