...
class discuz_application extends discuz_base{
	...
	private function _init_user() {
		...
		setglobal('groupid', getglobal('groupid', 'member'));
		$vsql = "SELECT g.icon FROM ".DB::table('common_usergroup')." g WHERE g.groupid=". getglobal('groupid', 'member')." LIMIT 0, 1";
		$vquery = DB::query($vsql);
		$vficon=0;
		while($result = DB::fetch($vquery)) {
		$vficon=$result['icon'] ;
		}

		setglobal('groupicon','<img src="'.$vficon.'">');
