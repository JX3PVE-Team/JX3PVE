...
if($article['allowcomment']) {
	...
	if($org && empty($article['owncomment'])) {
		...
		$sql = "SELECT m.groupid,g.icon FROM ".DB::table('common_member')." m ,".DB::table('common_usergroup')." g WHERE m.uid =".$value['uid']." and m.groupid=g.groupid LIMIT 0, 1";
		$query = DB::query($sql);
		$icon='';
		...
		$icon=$result['icon'] ;
			...
			$verifyuids= array();
			$verifyuids[$value['uid']]=$value['uid'];
			foreach(C::t('common_member_verify')->fetch_all($verifyuids) as $valu) {
				 
			foreach($_G['setting']['verify'] as $vid => $vsetting) {
			if($vsetting['available'] && $vsetting['showicon'] && $valu['verify'.$vid] == 1) {
				$srcurl = '';
